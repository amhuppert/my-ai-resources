...
description: Manage software complexity through deep modules, information
  hiding, and strategic programming. Use when the user mentions "module design",
  "API too complex", "shallow class", "complexity budget", "strategic vs
  tactical", "deep module", "information leakage", "pass-through method", "this
  code is over-engineered", or "simplify this design". Also trigger when
  reviewing an interface for simplicity, evaluating whether an abstraction is
  pulling its weight, deciding whether a comment is worth writing, or choosing
  between general-purpose and special-purpose approaches. Covers deep vs shallow
  modules, red flags for complexity, and comments as design documentation. For
  code quality, see clean-code. For architecture boundaries, see
  clean-architecture.
license: MIT
metadata:
  author: wondelai
...
Score nothing until every mechanism has earned its place. Before the diagnostics, list each mechanism, each state a public result can take, and each shared contract, with the present consumer that needs it or the envelope condition it handles, and the simpler alternative it beat. A consumer is a caller, journey, or approved obligation that exists today; "future automation", "a downstream module", or "callers may need" is not one. Cut or defer every row without a consumer, then score what survives.

The eight diagnostics below all reward depth and hiding; none asks whether the thing being hidden needs to exist. A design can pass every row and still be three times the size the problem needs, because deep modules are as easy to build around an imagined requirement as a real one. In one audited design, a self-scored 7.5 sat beside eleven internal contract types and a four-state outcome that no requirement consumed. A smaller design that passes fewer rows beats a larger one that passes more.

## Scoring

**Goal: the smallest design that passes.** When reviewing or creating a design, score the mechanisms that survived the proportionality gate by counting how many of the eight Quick Diagnostic rows they satisfy (≈1.25 points each), then sanity-check against the bands:
...
Always state the current score, the diagnostic rows that failed, and the specific change each one needs. At design time, before any code exists, the two practice rows (design discussion in reviews; 10-20% of time on improvement) are not applicable rather than failed; do not fill them with a promise.
...
| Does every mechanism, state, and contract have a present consumer? | Speculative generality; depth spent on imagined requirements | Cut or defer it before scoring; see Proportionality gate |
