# BAML (Basically, A Made-Up Language) TypeScript Reference Guide for AI Agents
...
This reference covers TypeScript-specific usage. BAML syntax (`.baml` files) is shared across all languages.
</Overview>
...
3. Creates TypeScript interfaces and typed client functions
...
generator target {
  ...
  // Options: "typescript", "typescript/react", "python/pydantic", "go", "ruby/sorbet"
  ...
  // Default client mode: "sync" or "async" (default: "async")
  default_client_mode "async"

  // Module format: "cjs" (CommonJS) or "esm" (ES modules)
  module_format "cjs"

  // Shell command to run after generation (e.g., formatters)
  on_generate "prettier --write baml_client/"
}
```
...
### @assert - Strict validation (throws BamlValidationError on failure)
...
import type { TweetAnalysis } from "./baml_client/types";
...
function main() {
  // Blocking call - no await needed
  const result = b.ClassifyTweets(["Hello!", "Check out this deal!"]);
...
**Note:** Streaming is NOT available with `sync_client`.

### Multimodal in Code

```typescript
import { Image, Audio, Video, Pdf } from "@boundaryml/baml";
import { b } from "./baml_client";
...
// Image from base64
const result = await b.DescribeImage(
  Image.fromBase64("image/png", base64String),
);

// Audio from URL
const result = await b.TranscribeAudio(
  Audio.fromUrl("https://example.com/audio.ogg"),
);

// Audio from base64
const result = await b.TranscribeAudio(
  Audio.fromBase64("audio/ogg", base64String),
);

// Video from URL
const result = await b.AnalyzeVideo(
  Video.fromUrl("https://example.com/video.mp4", "video/mp4"),
);

// PDF from URL
const result = await b.ParseDocument(
  Pdf.fromUrl("https://example.com/doc.pdf", "application/pdf"),
);

// PDF from base64
const result = await b.ParseDocument(
  Pdf.fromBase64("application/pdf", base64String),
);
```

#### Browser-Only Media Methods

```typescript
import { Audio, Video, Pdf } from "@boundaryml/baml";

// From File input element
const audio = await Audio.fromFile(fileInput.files[0]);
const video = await Video.fromFile(fileInput.files[0]);
const pdf = await Pdf.fromFile(fileInput.files[0]);

// From Blob
const audio = await Audio.fromBlob(blob, "audio/ogg");
const video = await Video.fromBlob(blob, "video/mp4");
const pdf = await Pdf.fromBlob(blob, "application/pdf");
```

#### Media Instance Methods

All media types (`Image`, `Audio`, `Video`, `Pdf`) share these methods:

| Method       | Returns            | Description                                   |
| ------------ | ------------------ | --------------------------------------------- |
| `isUrl()`    | `boolean`          | Whether stored as URL                         |
| `asUrl()`    | `string`           | Get URL (throws if base64)                    |
| `asBase64()` | `[string, string]` | Get `[base64Data, mediaType]` (throws if URL) |
| `toJSON()`   | `object`           | `{ url }` or `{ base64, media_type }`         |
...
for await (const partial of stream) {
  console.log(partial); // Partial<T> with optional fields
}
const final = await stream.getFinalResponse(); // Complete validated object
```
...
### Basic Concurrency with Promise.all
...
const promises = [
  b.ClassifyMessage("cancel order", { abortController: controller }),
  b.ClassifyMessage("refund", { abortController: controller }),
  b.ClassifyMessage("shipping?", { abortController: controller }),
];
...
### Cancelling Individual Operations
...
const promises = [
  b.ClassifyMessage("msg1", { abortController: controllers[0] }),
  b.ClassifyMessage("msg2", { abortController: controllers[1] }),
  b.ClassifyMessage("msg3", { abortController: controllers[2] }),
];
...
```typescript
// Auto-cancel after 5 seconds (Node.js 16+)
...
try {
  const result = await b.MyFunction(input, { abortController: controller });
} catch (error) {
...
| `message` | `string` | Description of the abort |
| `reason` | `any` | Optional context from `controller.abort(reason)` |
| `detailed_message` | `string` | Debugging information |

### BamlValidationError

Thrown when `@assert` validation fails.

```typescript
import { BamlValidationError } from "@boundaryml/baml";

try {
  const result = await b.GetScore(input);
} catch (error) {
...
generator typescript {
  ...
  output_dir "../"
  version "0.76.2"
  module_format "esm"            // Use ESM for Next.js
}
```
...
```typescript
// Use React-specific imports for browser media handling
...
| Method                   | Returns        | Description            |
| ------------------------ | -------------- | ---------------------- |
| `tb.string()`            | `FieldType`    | String type            |
| `tb.int()`               | `FieldType`    | Integer type           |
| `tb.float()`             | `FieldType`    | Float type             |
| `tb.bool()`              | `FieldType`    | Boolean type           |
| `tb.literal_string(val)` | `FieldType`    | Literal string type    |
| `tb.literal_int(val)`    | `FieldType`    | Literal int type       |
| `tb.literal_bool(val)`   | `FieldType`    | Literal bool type      |
| `tb.list(type)`          | `FieldType`    | List of type           |
| `tb.union(types)`        | `FieldType`    | Union of types         |
| `tb.map(key, value)`     | `FieldType`    | Map type               |
| `tb.addClass("Name")`    | `ClassBuilder` | Create new class       |
| `tb.addEnum("Name")`     | `EnumBuilder`  | Create new enum        |
| `tb.addBaml(schema)`     | `void`         | Add BAML schema inline |

### FieldType Methods

| Method        | Returns     | Description          |
| ------------- | ----------- | -------------------- |
| `.list()`     | `FieldType` | Convert to list type |
| `.optional()` | `FieldType` | Make optional        |

### ClassBuilder Methods

| Method                     | Returns                | Description      |
| -------------------------- | ---------------------- | ---------------- |
| `.addProperty(name, type)` | `ClassPropertyBuilder` | Add property     |
| `.description(text)`       | `ClassBuilder`         | Add description  |
| `.type()`                  | `FieldType`            | Get as FieldType |

### ClassPropertyBuilder Methods

| Method               | Returns                | Description     |
| -------------------- | ---------------------- | --------------- |
| `.description(text)` | `ClassPropertyBuilder` | Add description |
| `.alias(name)`       | `ClassPropertyBuilder` | Set alias       |

### EnumBuilder Methods

| Method               | Returns            | Description      |
| -------------------- | ------------------ | ---------------- |
| `.addValue(name)`    | `EnumValueBuilder` | Add value        |
| `.description(text)` | `EnumBuilder`      | Add description  |
| `.type()`            | `FieldType`        | Get as FieldType |
...
### Quick Client Override

```typescript
// Override client for a single call without ClientRegistry
...
| --------------------------------------- | -------------------------- | ----------------------- |
| `addLlmClient(name, provider, options)` | `(string, string, object)` | Add a new LLM client    |
| `setPrimary(name)`                      | `(string)`                 | Set which client to use |

**Supported providers:** `openai`, `anthropic`, `google-ai`, `vertex-ai`, `aws-bedrock`, `azure-openai`, `openai-generic`, `openai-responses`, `fallback`, `round-robin`
...
- TypeScript Setup: `docs.boundaryml.com/guide/installation-language/typescript`
...
- Concurrent Calls: `docs.boundaryml.com/guide/baml-basics/concurrent-calls`
- Error Handling: `docs.boundaryml.com/ref/baml-client/errors`
...
Run `npx baml-cli generate` after changes to regenerate the client code.

## Notes on Generated Types

- BAML types generate as TypeScript interfaces
- Union types generate discriminated unions
- Optional fields default to `undefined`
- Enum values are string literal unions (not TypeScript `enum`)
- The async client (`b`) returns `Promise<T>` for all functions
- The sync client returns `T` directly (no streaming support)
- Streaming yields `Partial<T>` where all fields are optional
