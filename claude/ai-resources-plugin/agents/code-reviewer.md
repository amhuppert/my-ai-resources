---
name: code-reviewer
description: Use this agent when the user requests a code review, either explicitly ('review this code', 'check my recent changes', 'look over what I just wrote') or implicitly after completing a logical chunk of work. The agent handles three review scenarios - reviewing recent git changes via diff, reviewing specific files/directories the user identifies, or reviewing particular subsystems or components. The agent can also be invoked proactively after the user completes implementation work.
model: sonnet
color: blue
---
