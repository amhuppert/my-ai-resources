...
description: "Characterize what a codebase is built for: infer its design envelope from the mechanisms in the code, compare it with the operating envelope it actually faces, and report where the code is overbuilt or underbuilt."
disable-model-invocation: true
argument-hint: '[path or area] [who runs it and where, e.g. "local CLI, only I run it"]'
---
...
- **Hostile data is in nearly every envelope, hostile layout is not.** A local tool that parses files fetched from the internet still treats those bytes as hostile: strict parsing, and never executing what it reads, is fit in every envelope. That is a different tier from defending against what the filesystem can do with those files (symlinks that escape a tree, hard links, names that collide under case folding or Unicode normalization, special files, metadata errors that are not ENOENT). Handling that tier is a mechanism; the single-operator default is to refuse such inputs with a plain error, and materializing, classifying, or recovering from them is a finding unless the envelope names a condition that needs it. In one audited design, the trust row's "imported files can be hostile" was read as the second tier and produced a filesystem owner that took ten of eighteen validation failures.
